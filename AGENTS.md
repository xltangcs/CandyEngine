...
- **渲染分层**（2026-08 RHI 收敛重构后）：
  ...
  - **HDR 管线**（2026-08）：场景 pass 渲染线性 HDR 到 `GameFrameRenderer` 自持的 `R16G16B16A16Float` 中间目标（`SceneRenderer::kSceneColorFormat`，所有场景 PSO 的 RT0 格式），随后 `TonemapPass`（`Tonemap.hlsl`：exposure → ACES → sRGB）合成到 LDR 显示目标（编辑器 viewport / CandyGame swap chain）；**game UI 在 tonemap 之后合成，不参与调色**；`SkyboxComponent::Exposure` 升格为全局曝光（经 `SceneRenderer::GetSkyboxExposure()` 传给 tonemap）；entity-id 拾取从 HDR target 附件 1 读取（`GameFrameRenderer::GetSceneColorTarget()`）；shader 一律输出线性 HDR，禁止 shader 内自带 Reinhard
