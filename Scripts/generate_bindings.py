...
# Default output for the component-inspector .inl file
DEFAULT_UI_OUTPUT = PROJECT_ROOT / "Candy" / "Source" / "Editor" / "Panel" / "ComponentUI.generated.inl"
# Default output for the component inventory (Copy/Duplicate lists)
DEFAULT_LISTS_OUTPUT = PROJECT_ROOT / "Candy" / "Source" / "Runtime" / "Scene" / "ComponentLists.generated.inl"
...
# C++ type -> ImGuiUtils control for the default inspector codegen.
UI_TYPE_MAP = {
    "float":                    "ImGuiUtils::DrawDragFloat",
    "double":                   "ImGuiUtils::DrawDragFloat",
    "int":                      "ImGuiUtils::DrawInputInt",
    "int32_t":                  "ImGuiUtils::DrawInputInt",
    "uint32_t":                 "ImGuiUtils::DrawInputInt",
    "bool":                     "ImGuiUtils::DrawCheckbox",
    "std::string":              "ImGuiUtils::DrawInputText",
    "glm::vec2":                "ImGuiUtils::DrawDragFloat2",
    "glm::vec3":                "ImGuiUtils::DrawVec3Control",
    "glm::vec4":                "ImGuiUtils::DrawColorEdit4",
    "std::vector<std::string>": "ImGuiUtils::DrawMultilineStringList",
}

# Components registered manually in PythonBindings.cpp — the codegen must not
# emit a second py::class_ for them (pybind11 forbids re-registration).
PYTHON_MANUAL_BINDINGS = {
    "Rigidbody2DComponent",
    "UITextBlockComponent",
    "UIButtonComponent",
}

# Components excluded from the auto-generated Copy/Duplicate lists
# (managed directly by the scene lifecycle).
COPY_EXCLUDED_COMPONENTS = {
    "IDComponent",       # identity, always re-created
    "TagComponent",      # name passed to CreateEntityWithUUID
    "TextBlockUIData",   # nested data structure, not an entity component
    "ButtonUIData",      # nested data structure, not an entity component
}
...
def generate_inl(structs: list[StructInfo], enums: list[EnumInfo], output_path: Path):
    ...
    lines.append("// Generated: deterministic (see git)")
    lines.append("")
    lines.append("// ============================================================================")
    ...
    for s in structs:
        if s.name in PYTHON_MANUAL_BINDINGS:
            continue
        cpp_type = s.cpp_type
...
def generate_component_ui(structs: list[StructInfo], output_path: Path):
    """Generate ComponentUI.generated.inl — default ImGui inspector bodies
    driven by CANDY_PROPERTY metadata.

    This is the *starting point* for a new component's inspector: field types
    map to generic controls (drag floats, checkboxes, text inputs, color
    pickers). Components that need semantic controls (asset-path inputs with
    load callbacks, enums as combos, unit conversions) keep their hand-written
    DrawComponent<> lambda in SceneHierarchyPanel.cpp; the generated default
    covers the remaining fields. Fields without a UI mapping are skipped.
    """
    lines = []
    lines.append("// Auto-generated by generate_bindings.py - DO NOT EDIT")
    lines.append("// Generated: deterministic (see git)")
    ...
    for s in structs:
        drawable = [(mt, mn) for mt, mn in s.members if mt in UI_TYPE_MAP]
        if not drawable:
            continue
        ...
        lines.append(f"// ---- {s.name} ------------------------------------------------")
        lines.append(f"inline void Draw{s.name}Fields({cpp_type}& c)")
        lines.append("{")
        for mtype, mname in drawable:
            lines.append(f"\t{UI_TYPE_MAP[mtype]}(\"{mname}\", c.{mname});")
        lines.append("}")
        lines.append("")
...
def generate_component_lists(structs: list[StructInfo], output_path: Path):
    """Generate ComponentLists.generated.inl — the component inventory as two
    macros consumed by Scene::Copy / Scene::DuplicateEntity, so adding a
    component never means remembering to extend those hand-written lists.
    """
    copyable = [s for s in structs if s.name not in COPY_EXCLUDED_COMPONENTS]
    ...
    lines.append("// One CopyComponent<T> per entity component (except ID/Tag, which")
    lines.append("// the scene re-creates itself).")
    lines.append("#define CANDY_COPY_COMPONENT_LIST(dst, src, enttMap) \\")
    for i, s in enumerate(copyable):
        suffix = "" if i == len(copyable) - 1 else " \\"
        lines.append(f"\tCopyComponent<Candy::{s.name}>(dst, src, enttMap);{suffix}")
    lines.append("")
    lines.append("// One CopyComponentIfExists<T> per entity component.")
    lines.append("#define CANDY_DUPLICATE_COMPONENT_LIST(dst, src) \\")
    for i, s in enumerate(copyable):
        suffix = "" if i == len(copyable) - 1 else " \\"
        lines.append(f"\tCopyComponentIfExists<Candy::{s.name}>(dst, src);{suffix}")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--ui-output", default=str(DEFAULT_UI_OUTPUT),
                        help="Output path for ComponentUI.generated.inl")
    parser.add_argument("--lists-output", default=str(DEFAULT_LISTS_OUTPUT),
                        help="Output path for ComponentLists.generated.inl")
    ...
    ui_path = Path(args.ui_output)
    lists_path = Path(args.lists_output)
    ...
    # Generate default inspector bodies (CANDY_PROPERTY driven)
    generate_component_ui(structs, ui_path)

    # Generate the component inventory (Copy/Duplicate lists)
    generate_component_lists(structs, lists_path)
