...
namespace Candy {
	...
	public:
		...
		/// Call after Present to signal the resize fence so a subsequent Resize()
		/// can safely drain the presented frame before ResizeBuffers.
		void SignalAfterPresent();
	...
	private:
		...
		// Flags the swap chain was created with. Flip-model ResizeBuffers requires
		// the same flags be passed back (otherwise E_INVALIDARG).
		UINT m_CreationFlags = 0;

		// Resize synchronization. SignalAfterPresent() signals m_ResizeFence on
		// m_Queue right after Present, so the fence covers both the command list
		// AND the display-pipeline present. Resize() waits on it before
		// ResizeBuffers — otherwise a still-pending present makes DXGI return
		// DXGI_ERROR_DEVICE_REMOVED (0x887A0005).
		Microsoft::WRL::ComPtr<ID3D12Fence> m_ResizeFence;
		HANDLE                             m_ResizeFenceEvent = nullptr;
