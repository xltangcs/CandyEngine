...
namespace Candy {
	...
	void D3D12CommandBuffer::BeginRenderPass(const RenderPassDesc& desc)
	{
		...
		if (m_CurrentFramebuffer)
		{
			...
			// Transition color attachments to RENDER_TARGET. The previous editor
			// ImGui pass read attachment 0 as PIXEL_SHADER_RESOURCE (the viewport
			// image), so D3D12 requires an explicit barrier back to RENDER_TARGET
			// before rendering into it again.
			for (uint32_t i = 0; i < colorCount; ++i)
				m_CurrentFramebuffer->EnsureColorAttachmentState(m_CommandList.Get(), i, D3D12_RESOURCE_STATE_RENDER_TARGET);
		...
		// Clear color attachments — honor LoadOp: only attachments explicitly
		// marked LoadOp::Clear are cleared. Passes that accumulate onto an
		// already-rendered target (e.g. the editor overlay pass) use LoadOp::Load
		// and must not erase existing content.
	...
	void D3D12CommandBuffer::EndRenderPass()
	{
		if (m_CurrentFramebuffer)
		{
			...
			// ImGui pass can read it as the viewport image (SRV). D3D12 requires an
			// explicit RENDER_TARGET → PIXEL_SHADER_RESOURCE barrier here — without it
			// the GPU reads the texture in the wrong state (garbage viewport / hang).
			if (m_CurrentFramebuffer->GetColorAttachmentCount() > 0)
				m_CurrentFramebuffer->EnsureColorAttachmentState(m_CommandList.Get(), 0, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			return;
		}
