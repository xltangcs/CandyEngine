	...
	namespace {
		...
		Ref<Material> ImportMaterial(const cgltf_material* mat, const std::string& baseDir)
		{
			...
			if (mat->has_pbr_metallic_roughness)
			{
				...
				material->BaseColorMap = TextureVfsPath(pbr.base_color_texture, baseDir);
				material->MetallicRoughnessMap = TextureVfsPath(pbr.metallic_roughness_texture, baseDir);
			}
			...
			material->NormalMap   = TextureVfsPath(mat->normal_texture, baseDir);
			material->EmissiveMap = TextureVfsPath(mat->emissive_texture, baseDir);

			switch (mat->alpha_mode)
			{
				case cgltf_alpha_mode_mask:  material->BlendMode = MaterialBlendMode::Masked;      break;
				case cgltf_alpha_mode_blend: material->BlendMode = MaterialBlendMode::Transparent; break;
				default:                     material->BlendMode = MaterialBlendMode::Opaque;      break;
			}
